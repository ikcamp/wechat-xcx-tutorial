...
*/
    ...
    if (data && data.length) {
      formatData = data.map((group) => {
        ...
        if (group && group.articles) {
          if (group.formateDate === '今日' && group.articles && group.articles.length) {
          // if (true && group.articles && group.articles.length) {  
            promotArticle = group.articles[0];
            group.articles = group.articles.slice(1);
          }
  ...
  */
  ...
  renderArticle(data) {
    if (data) {
      let newArticles = data.formatData || [];
      let newList = this.data.articleList.concat(newArticles);
      this.setData({
        promotArticle: data.promotArticle,
        articleList: newList,
        hiddenLoading: true
      });
    }
